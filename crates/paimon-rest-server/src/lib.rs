...
/// Alter database: FileSystemCatalog does not persist database properties (see
/// `docs/filesystem-catalog-gaps.md`), and the `Catalog` trait has no
/// `alter_database`, so no client path reaches this. We only validate that the
/// database exists and return OK; the request is intentionally a no-op.
