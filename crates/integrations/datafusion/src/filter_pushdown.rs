...
impl<'a> FilterTranslator<'a> {
    ...
    fn translate_between(&self, between: &Between) -> Option<Predicate> {
        ...
        if between.negated {
            // Same concern as Expr::Not: NOT BETWEEN's three-valued logic
            // (NULL → unknown → false) is incompatible with paimon's
            // Predicate::Not / NotBetween Exact-pushdown contract. Keep
            // pessimistic until we revisit three-valued handling.
            return None;
        }

        // Native Between leaf: lets the planner / b-tree recognize the range
        // as a single op (see `btree::query::extract_between`).
...
mod tests {
    ...
    fn test_case_insensitive_pushdown_translates_to_canonical_name() {
        ...
        assert!(
            !analysis.has_untranslated_residual,
