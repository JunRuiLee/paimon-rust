...
impl PredicateBuilder {
    ...
    /// `field LIKE 'pat%'` shape. Empty pattern → [`Predicate::AlwaysTrue`]
    /// (every string starts with the empty string). Non-string `pattern`
    /// → [`Error::ConfigInvalid`].
    ...
    ///
    /// `escape == None` defaults to `\`. ESCAPE characters other than `\` are
    /// not yet rewriteable and surface as a `Like` leaf with the alternative
    /// escape carried in the literal — currently rejected, leaving the caller
    /// to fall open. Empty pattern → [`PredicateOperator::Eq`] of the empty
    /// string (SQL semantics: only the empty string matches).
    pub fn like(
        &self,
        field: &str,
        pattern: Datum,
    ...
    fn string_leaf(
        ...
        match &pattern {
            Datum::String(s) if s.is_empty() => return Ok(Predicate::AlwaysTrue),
            Datum::String(_) => {}
            other => {
                return Err(Error::ConfigInvalid {
...
fn optimize_like_pattern(pattern: &str) -> LikeShape {
    ...
    match percent_count {
        ...
        2 if pattern.starts_with('%') && pattern.ends_with('%') => {
            // `%%` reduces to `Contains('')`, which itself short-circuits
            // to AlwaysTrue at the StartsWith/EndsWith/Contains builder
            // boundary — so no special-casing here.
...
/// matcher used by `arrow_string::like::like` (and aligned with Java
/// `org.apache.paimon.predicate.Like`):
/// * `%` matches any (possibly empty) substring,
/// * `_` matches exactly one character,
/// * `\X` matches the literal `X` (only `\\`, `\%`, `\_` carry meaning;
///   stray `\X` is treated as a literal `\` followed by `X`, mirroring
///   arrow's tolerant behavior).
...
mod tests {
    ...
    #[test]
    fn test_builder_string_ops_empty_pattern_short_circuits_to_always_true() {
        let pb = PredicateBuilder::new(&test_fields());
        for build in [
            ...
            assert!(matches!(build.unwrap(), Predicate::AlwaysTrue));
        }
    }
