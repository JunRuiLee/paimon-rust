...
fn evaluate_arrow_leaf_predicate(
    ...
    match op {
        ...
        // String / range operators are not evaluated at the Vortex filter
        // layer: returning `None` leaves the predicate for the outer (DataFusion
        // inexact) filter to re-apply, preserving correctness. This keeps the
        // operators internal `predicate.rs` exposes (F7 leaf extension) working
        // after #387 replaced Vortex-expression pushdown with read-side masks.
...
fn evaluate_column_predicate(
    ...
    match op {
        ...
        // These operators never reach the column evaluator: set membership is
        // handled by `evaluate_set_membership_predicate`, and string / range
        // operators fall through to the outer filter (see
        // `evaluate_arrow_leaf_predicate`). Return an all-null mask for the
